...
/*
    ...
    목적:
    - relay_proxy:8000 으로 CONNECT 요청을 보낸다.
    - CONNECT 성공 후 TLS handshake를 수행한다.
    - 같은 TLS 세션 안에서 HTTP request를 여러 번 보낸다.
    ...
    기대 relay_proxy 로그:
    - exchange_index=1 처리 후 keep-alive continues
    - exchange_index=2 처리 후 keep-alive continues
    - exchange_index=3 에서 secret 탐지 후 response blocked
*/
