...
static SOCKET connectUpstreamTcp(const std::string& host, int port) {
    ...
    // AF_UNSPEC: IPv4·IPv6 둘 다 허용. IPv4로 고정하면 IPv6 전용 호스트에 도달 못 한다
    //   (실측: DeepSeek의 파일 처리 백엔드 hif-dliq.deepseek.com 은 A(IPv4) 레코드가
    //    없고 AAAA(IPv6)만 있어, AF_INET 고정 시 getaddrinfo가 주소를 못 줘 업로드가
    //    통째로 실패했다 — deepseek_web/file_B.md, grok-websocket-limitation.md 계열 기록).
    hints.ai_family = AF_UNSPEC;
    hints.ai_socktype = SOCK_STREAM;
    std::string portStr = std::to_string(port);
    if (getaddrinfo(host.c_str(), portStr.c_str(), &hints, &info) != 0) return INVALID_SOCKET;

    // getaddrinfo는 여러 주소를 리스트로 준다(예: AAAA 먼저, A 나중). 첫 번째만 쓰면
    // 그게 실패할 때 나머지를 못 써본다. 연결될 때까지 순회한다.
